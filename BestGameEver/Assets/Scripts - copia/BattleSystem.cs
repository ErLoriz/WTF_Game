...
public class BattleSystem : MonoBehaviour
{
    ...
    //ComboJugador
    public GameObject comboJugador;
    public int aguaJugador;
    public int fuegoJugador;
    public int tierraJugador;
    ...
    public void OnClickButtonTurn()
    {
        ...
        if (state == BattleState.PLAYERTURN)
        {
            ...
            if (cont == 1)
            {
                ...
                //Habilidad agua
                comboJugador = GameObject.Find("ComboPanelElemento");
                 aguaJugador = comboJugador.GetComponent<ComboPanelObjeto>().getAgua();
                //Agua
                CartasArray = GameObject.FindGameObjectsWithTag("CartaCampo");
              

                if (aguaJugador >= 6) //6- Cura 3 vida a todas las cartas del campo y jugador
                {
               
                    Jugador.GetComponent<ObjetoJugador>().ganarVida(3);
    ...
    async void EnemyTurn()
    {
        ...
        combosJugador(GameObject.FindGameObjectsWithTag("CartaCampo"));
        ...
        comboJugador = GameObject.Find("ComboPanelElemento");
        tierraJugador = comboJugador.GetComponent<ComboPanelObjeto>().getTierra();

        CartasArray = GameObject.FindGameObjectsWithTag("Campo");
        if (tierraJugador >= 2)
        {
            tierraEscudoJugador(CartasArray);
        }
    ...
    public async void combate(GameObject CartaIASelect, GameObject CartaJugadorSelect) //Se le pasan dos cartas 
    {
        ...
        else
        {
            ...
            CartaJugadorSelect.GetComponent<ObjetoCarta>().perderVida(CartaIASelect.GetComponent<ObjetoCarta>().getAtaque() - CartaJugadorSelect.GetComponent<ObjetoCarta>().getEscudo());
            comboJugador = GameObject.Find("ComboPanelElemento");
            fuegoJugador = comboJugador.GetComponent<ComboPanelObjeto>().getFuego();
           
            if (fuegoJugador >=2 && CartaJugadorSelect.GetComponent<ObjetoCarta>().getElemento().ToString().Equals("Fuego"))
            {
                IraJugador(CartaJugadorSelect, CartaIASelect);
            }
    ...
    void combosJugador(GameObject[] cartasArray)
    {
        Aire = 0; Agua = 0; Tierra = 0; Fuego = 0; Luz = 0; Oscuridad = 0;
    ...
    void tierraEscudoJugador(GameObject[] arrayCartasJugador)
    {
        Debug.Log("Pues na aqui se acticvan los escudos: " + tierraJugador);
        if (tierraJugador >= 4)
        {
            for (int i = 0; i < arrayCartasJugador.Length; i++)
            {
                    arrayCartasJugador[i].GetComponent<ObjetoCarta>().setEscudo(2);
            }
        }
        else
        {
            for (int i = 0; i < arrayCartasJugador.Length; i++)
            {
                if (arrayCartasJugador[i].GetComponent<ObjetoCarta>().getElemento().ToString().Equals("Tierra"))
                {
                    arrayCartasJugador[i].GetComponent<ObjetoCarta>().setEscudo(2);
                }
            }
    ...
    void IraJugador(GameObject cartaJugadorSelect, GameObject cartaIASelect)
    {
        Debug.Log("Entro en iraJugador desde IA");
        if (fuegoJugador >= 4)
        {
